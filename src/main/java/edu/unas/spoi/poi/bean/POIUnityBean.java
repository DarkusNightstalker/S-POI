...
/**
 * The type Poi unity bean.
 *
 * @author Jhoan Brayam
 */
@ManagedBean(name = "poiUnityBean")
@SessionScoped
public class POIUnityBean implements java.io.Serializable,ILoadable{
    ...
    public class Import implements java.io.Serializable {
        ...
        @Asynchronous
        public void begin() {
            List<POIUnity> list = new ArrayList();
            try {
                Object file = null;
                try {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    poiUnityService.saveOrUpdate(list.get(i));
                }
    ...
    public class Export implements java.io.Serializable {
        ...
        private String execute(JRExporter exporter) {
            try {
                ...
                InputStream is = servletContext.getResourceAsStream("/1258488425132154132154214536/POIUnity.jasper");
                ...
                response.addHeader("Content-disposition", "attachment; filename=Unidades_POI." + contentType.name().toLowerCase());
